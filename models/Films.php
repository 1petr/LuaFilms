...
class Film
{
    ...
    public static function getListFilms()
    {
       ...
       while($row = $result->fetch())
       {
		   ...
		   $filmList[$i]['date'] = $row['date'];
		   $filmList[$i]['duration'] = $row['duration'];
    ...
    public static function getListFilmsGenre($sort)
    {
       ...
       $db = Db::getConnection();
        ...
        //Выбираем все из бд
       $result = $db->query('SELECT * FROM films ORDER BY id DESC');
       ...
       while($row = $result->fetch())
       {
		   ...
		   $filmListSort[$i]['date'] = $row['date'];
		   $filmListSort[$i]['duration'] = $row['duration'];
           /*echo '<pre>';
...
public static function getSeral($name)
{
	   //Подключаемся к бд
       $db = Db::getConnection();
        
       //Выбираем все из бд
       $result = $db->query('SELECT * FROM serials ORDER BY id DESC');
       ...
       while($row = $result->fetch())
       {
		   ...
		   $filmListSort[$i]['date'] = $row['date'];
		   $filmListSort[$i]['duration'] = $row['duration'];
		   $filmListSort[$i]['season'] = $row['season'];
		   $filmListSort[$i]['series'] = $row['series'];
           /*echo '<pre>';
            print_r($filmListGenre[$i]['id']);
            echo '</pre>';*/
           $i++;
...
public static function getSerialsItemById($id)
{
	$id = intval($id);
    $db = Db::getConnection(); 
    $result = $db->query('SELECT * FROM serials WHERE id='.$id);
    $result->setFetchMode(PDO::FETCH_ASSOC);
    $filmItem = $result->fetch();
...
public static function setRating($id, $rating, $num)
{
    $db = new Db();
	$updateRow = $db->updateRow("UPDATE films SET rating = ?, num-rating = ? WHERE id = ?", ["2","5","1"]);
}
